...
func (m *Repository) AdminReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	fistOfMonth := time.Date(currentYear, currentMonth, 1, 0, 0, 0, 0, currentLocation)
	lastOfMonth := fistOfMonth.AddDate(0, 1, -1)
	...
	for _, x := range rooms {
		...
		for d := fistOfMonth; !d.After(lastOfMonth); d = d.AddDate(0, 0, 1) {
			reservationMap[d.Format("2006-01-2")] = 0
			blockMap[d.Format("2006-01-2")] = 0
		}

		restrictions, err := m.DB.GetRestrictionsForRoomByDate(x.ID, fistOfMonth, lastOfMonth)
		if err != nil {
			helpers.ServerError(w, err)
			return
		}
		...
		for _, y := range restrictions {
			if y.ReservationID > 0 {
				...
				blockMap[y.StartDate.Format("2206-01-2")] = y.ID
			}
